...
func Test_Caravan_Purge_Line_First(t *testing.T) {
	c, _ := createLine(t)

	c.Purge(func(h Hash, _ *Node) bool {
		return h == 0xf0a
	})
	...
	if len(c.nodes) != 2 {
		t.Errorf("Expected to find %d nodes; have %d", 2, len(c.nodes))
	}

	if _, ok := c.Find(0xf0a); ok {
		t.Error("After purging, node is still findable")
	}

	n, _ := c.Find(0xf1a)
	if len(n.Ascendants) != 0 {
		t.Errorf("Child node still has purged node as ascendant")
	}

	if _, ok := c.roots[0xf0a]; ok {
		t.Errorf("Root nodes collection still has purged node")
	}

	if _, ok := c.roots[0xf1a]; !ok {
		t.Errorf("Child node did not being root node")
	}
}

func Test_Caravan_Purge_Line_Last(t *testing.T) {
	c, _ := createLine(t)
	...
	if _, ok := c.Find(0xf2a); ok {
		t.Error("After purging, node is still findable")
	}

	n, _ := c.Find(0xf1a)
	if len(n.Descendants) != 0 {
		t.Errorf("Parent node still has purged node as descentant")
	}
}
...
func createLine(t *testing.T) (*Caravan, []Hasher) {
	// f0a
	//  |
	// f1a
	//  |
