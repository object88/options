...
func (c *Caravan) Ensure(hash Hash, create func() Hasher) (*Node, bool) {
	...
	if !ok {
		...
		n = c.insert(newP)
		if newP.Hash() != hash {
			message := fmt.Sprintf("Attempt to add a Hasher whose hash (0x%016x) does not match the provided hash (0x%016x)", newP.Hash(), hash)
			panic(message)
		}
	}
	c.m.Unlock()
...
// Purge cycles through all nodes, and removes them if the provided predicate
// function returns true
func (c *Caravan) Purge(pred func(Hash, *Node) bool) {
	c.Walk(WalkUp, func(n *Node) {
		h := n.Element.Hash()
		if pred(h, n) {
			if _, ok := c.roots[h]; ok {
				delete(c.roots, h)
			}
			delete(c.nodes, n.Element.Hash())
			for _, p := range n.Descendants {
				delete(p.Ascendants, h)
				if len(p.Ascendants) == 0 {
					c.roots[p.Element.Hash()] = p
				}
			}
			for _, p := range n.Ascendants {
				delete(p.Descendants, h)
			}
		}
	})
}
